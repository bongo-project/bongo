...
Connection *
MsgNmapConnect(const char *authUser, const char *store, MDBValueStruct *optVs)
{
    return MsgNmapConnectEx(authUser, store, optVs, NULL);
}

Connection *
MsgNmapConnectEx(const char *authUser, const char *store, MDBValueStruct *optVs, TraceDestination *dest)
{
    ...
    if (optVs) {
        ...
        vs = MDBCreateValueStruct(MsgGlobal.directoryHandle, NULL);
    }

    success = FALSE;
    conn = NULL;
    
    if (MsgFindObject(store, NULL, NULL, &saddr, vs)) {
        MDBFreeValues(vs);
        
        conn = NMAPConnectEx(NULL, &saddr, dest);
