...
static const
unsigned short ISO8859_16_UTF8[96] = {
  /* 0xa0 */
...
static int
ISO8859_16_Decode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	...
	while (Len<Codec->Len) {
		...
		if (*In<0xa0) {
			*Out=*In;
			...
			UTF8EncodeChar(ISO8859_16_UTF8[*In - 0xa0], Out, Out, NextCodec->Len);
			Len++;
			In++;
		}
	}
...
static const
unsigned char ISO8859_16_CP00[224] = {
...
static const
unsigned char ISO8859_16_CP02[8] = {
  0xaa, 0xba, 0xde, 0xfe, 0x00, 0x00, 0x00, 0x00, /* 0x18-0x1f */
};

static const
unsigned char ISO8859_16_CP20[8] = {
  0x00, 0x00, 0x00, 0x00, 0x00, 0xb5, 0xab, 0x00, /* 0x18-0x1f */
};

static int
ISO8859_16_Encode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	unsigned char	*In=Codec->Start;
	unsigned char	*Out=NextCodec->Start+NextCodec->Len;
	unsigned char	*OutEnd=NextCodec->End;
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x0080) {
			...
			if (wc < 0x00a0){
				...
				Out[0] = (unsigned char)ISO8859_16_CP00[wc-0x00a0];
			} else if (wc >= 0x0218 && wc < 0x0220) {
				Out[0] = (unsigned char)ISO8859_16_CP02[wc-0x0218];
			} else if (wc >= 0x2018 && wc < 0x2020) {
				Out[0] = (unsigned char)ISO8859_16_CP20[wc-0x2018];
			} else if (wc == 0x20ac) {
				Out[0] = (unsigned char)0xa4;
			} else {
				Out[0] = '?';
			}
			Out++;
