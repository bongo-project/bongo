...
static int
CP874_Decode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x80) {
			*Out=*In;
			...
			UTF8EncodeChar(cp874_2uni[In[0]-0x80], Out, Out, NextCodec->Len);
			Len++;
			In++;
		}
	}
...
static int
CP874_Encode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	unsigned char	*In=Codec->Start;
	unsigned char	*Out=NextCodec->Start+NextCodec->Len;
	unsigned char	*OutEnd=NextCodec->End;
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x0080) {
			...
			if (wc>=0x0e00 && wc<0x0e60) {
				Out[0]=cp874_page0e[wc-0x0e00];
			} else if (wc>=0x2010 && wc<0x2028) {
				Out[0]=cp874_page20[wc-0x2010];
			} else if (wc==0x00a0) {
				Out[0]=0xa0;
			} else if (wc==0x20ac) {
				Out[0]=0x80;
