...
static int
CP1254_Decode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x80) {
			*Out=*In;
			...
			if (In[0]<0xa0) {
				UTF8EncodeChar(cp1254_2uni_1[In[0]-0x80], Out, Out, NextCodec->Len);
			} else if (In[0]<0xd0) {
				Out[0]=In[0];
				Out++;
				NextCodec->Len++;
			} else if (In[0]<0xe0) {
				UTF8EncodeChar(cp1254_2uni_2[In[0]-0xd0], Out, Out, NextCodec->Len);
			} else if (In[0]<0xf0) {
				Out[0]=In[0];
				Out++;
				NextCodec->Len++;
			} else {
				UTF8EncodeChar(cp1254_2uni_3[In[0]-0xf0], Out, Out, NextCodec->Len);
			}
			Len++;
			In++;
		}
...
static int
CP1254_Encode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	unsigned char	*In=Codec->Start;
	unsigned char	*Out=NextCodec->Start+NextCodec->Len;
	unsigned char	*OutEnd=NextCodec->End;
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x0080) {
			...
			if (wc>=0x00a0 && wc<0x00d0) {
				Out[0]=(unsigned char)wc;
			} else if (wc>=0x00d0 && wc<0x0100) {
				Out[0]=cp1254_page01[wc-0x00d0];
			} else if (wc>=0x0118 && wc<0x0198) {
				Out[0]=cp1254_page01[wc-0x0118];
			} else if (wc>=0x02c0 && wc<0x02e0) {
				Out[0]=cp1254_page02[wc-0x02c0];
			} else if (wc>=0x2010 && wc<0x2040) {
				Out[0]=cp1254_page20[wc-0x2010];
			} else if (wc==0x20ac) {
				Out[0]=0x80;
			} else if (wc==0x2122) {
				Out[0]=0x99;
