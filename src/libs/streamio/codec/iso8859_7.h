...
static const
unsigned short ISO8859_7_UTF8[96] = {
  /* 0xa0 */
...
static int
ISO8859_7_Decode(StreamStruct *Codec, StreamStruct *NextCodec)
{
...
static const
unsigned char ISO8859_7_CP00[32] = {
...
static const
unsigned char ISO8859_7_CP03[80] = {
...
static const
unsigned char ISO8859_7_CP20[16] = {
...
static int
ISO8859_7_Encode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	unsigned char	*In=Codec->Start;
	unsigned char	*Out=NextCodec->Start+NextCodec->Len;
	unsigned char	*OutEnd=NextCodec->End;
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x0080) {
			...
			if (wc <0x00a0) {
				...
				Out[0] = ISO8859_7_CP00[wc-0x00a0];
			} else if (wc >= 0x0380 && wc < 0x03d0) {
				Out[0] = ISO8859_7_CP03[wc-0x0380];
			} else if (wc >= 0x2010 && wc < 0x2020) {
				Out[0] = ISO8859_7_CP20[wc-0x2010];
			} else {
				Out[0] = '?';
			}
			Out++;
