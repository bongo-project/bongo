...
static const
unsigned short ISO8859_3_UTF8[96] = {
  /* 0xa0 */
...
static int
ISO8859_3_Decode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	...
	while (Len<Codec->Len) {
		...
		if (*In<0xa0) {
			*Out=*In;
			...
			if (ISO8859_3_UTF8[*In - 0xa0] != 0xfffd) {
				UTF8EncodeChar(ISO8859_3_UTF8[*In - 0xa0], Out, Out, NextCodec->Len);
			}
			Len++;
			In++;
		}
...
static const
unsigned char ISO8859_3_CP00[96] = {
...
static const
unsigned char ISO8859_3_CP01[120] = {
...
static const
unsigned char ISO8859_3_CP02[8] = {
  0xa2, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, /* 0xd8-0xdf */
};

static int
ISO8859_3_Encode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	unsigned char	*In=Codec->Start;
	unsigned char	*Out=NextCodec->Start+NextCodec->Len;
	unsigned char	*OutEnd=NextCodec->End;
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x0080) {
			...
			if (wc < 0x00a0){
				...
				Out[0] = (unsigned char)ISO8859_3_CP00[wc-0x00a0];
			} else if (wc >=0x0108 && wc < 0x0180) {
				Out[0] = (unsigned char)ISO8859_3_CP01[wc-0x0108];
			} else if (wc >= 0x02d8 && wc < 0x02e0) {
				Out[0] = (unsigned char)ISO8859_3_CP02[wc-0x02d8];
			} else {
				Out[0] = '?';
			}
			Out++;
