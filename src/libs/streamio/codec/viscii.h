...
static int
VISCII_Decode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	...
	while (Len<Codec->Len) {
		FlushOutStream(5);
		if (In[0]<0x20) {
			UTF8EncodeChar(viscii_2uni_1[In[0]], Out, Out, NextCodec->Len);
			Len++;
			In++;
		} else if (In[0]<0x80) {
			*Out=*In;
			...
			UTF8EncodeChar(viscii_2uni_2[In[0]-0x80], Out, Out, NextCodec->Len);
			Len++;
			In++;
		}
	}
...
static int
VISCII_Encode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	unsigned char	*In=Codec->Start;
	unsigned char	*Out=NextCodec->Start+NextCodec->Len;
	unsigned char	*OutEnd=NextCodec->End;
	...
	while (Len<Codec->Len) {
		...
		if (In[0]<0x0080) {
			...
			if (wc>=0x0020 || (0x42100064 & (1<<wc))==0) {
				Out[0]=In[0];
			} else if (wc>=0x00c0 && wc<0x01b8) {
				Out[0]=viscii_page00[wc-0x00c0];
			} else if (wc>=0x1ea0 && wc<0x1f00) {
				Out[0]=viscii_page1e[wc-0x1ea0];
			} else {
				continue;
			}
			Out++;
