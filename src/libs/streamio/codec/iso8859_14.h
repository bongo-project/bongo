...
static const
unsigned short ISO8859_14_UTF8[96] = {
  /* 0xa0 */
...
static int
ISO8859_14_Decode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	...
	while (Len<Codec->Len) {
		...
		if (*In<0xa0) {
			*Out=*In;
			...
			UTF8EncodeChar(ISO8859_14_UTF8[*In - 0xa0], Out, Out, NextCodec->Len);
			Len++;
			In++;
		}
	}
...
static const
unsigned char ISO8859_14_CP00[96] = {
...
static const
unsigned char ISO8859_14_CP01_0[32] = {
...
static const
unsigned char ISO8859_14_CP1E_1[8] = {
  0x00, 0x00, 0xac, 0xbc, 0x00, 0x00, 0x00, 0x00, /* 0xf0-0xf7 */
};

static int
ISO8859_14_Encode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	unsigned char	*In=Codec->Start;
	unsigned char	*Out=NextCodec->Start+NextCodec->Len;
	unsigned char	*OutEnd=NextCodec->End;
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x0080) {
			...
			if (wc < 0x00a0){
				...
				Out[0] = (unsigned char)ISO8859_14_CP00[wc-0x00a0];
			} else if (wc >= 0x0108 && wc < 0x0128) {
				Out[0] = (unsigned char)ISO8859_14_CP01_0[wc-0x0108];
			} else if (wc >= 0x0170 && wc < 0x0180) {
				Out[0] = (unsigned char)ISO8859_14_CP01_1[wc-0x0170];
			} else if (wc >= 0x1e00 && wc < 0x1e88) {
				Out[0] = (unsigned char)ISO8859_14_CP1E_0[wc-0x1e00];
			} else if (wc >= 0x1ef0 && wc < 0x1ef8) {
				Out[0] = (unsigned char)ISO8859_14_CP1E_1[wc-0x1ef0];
			} else {
				Out[0] = '?';
			}
			Out++;
