...
static int
CP1250_Decode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x80) {
			*Out=*In;
			...
			UTF8EncodeChar(cp1250_2uni[In[0]-0x80], Out, Out, NextCodec->Len);
			Len++;
			In++;
		}
	}
...
static int
CP1250_Encode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	unsigned char	*In=Codec->Start;
	unsigned char	*Out=NextCodec->Start+NextCodec->Len;
	unsigned char	*OutEnd=NextCodec->End;
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x0080) {
			...
			if (wc>=0x00a0 && wc< 0x0180) {
				Out[0]=cp1250_page00[wc-0x00a0];
			} else if (wc>=0x02c0 && wc<0x02e0) {
				Out[0]=cp1250_page02[wc-0x02c0];
			} else if (wc>=0x2010 && wc<0x2040) {
				Out[0]=cp1250_page20[wc-0x2010];
			} else if (wc==0x20ac) {
				Out[0]=0x80;
			} else if (wc==0x2122) {
				Out[0]=0x99;
