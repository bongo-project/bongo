...
static int
CP1255_Decode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x80) {
			*Out=*In;
			...
			UTF8EncodeChar(cp1255_2uni[In[0]-0x80], Out, Out, NextCodec->Len);
			Len++;
			In++;
		}
	}
...
static int
CP1255_Encode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	unsigned char	*In=Codec->Start;
	unsigned char	*Out=NextCodec->Start+NextCodec->Len;
	unsigned char	*OutEnd=NextCodec->End;
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x0080) {
			...
			if (wc>=0x00a0 && wc<0x00f8) {
				Out[0]=cp1255_page00[wc-0x00a0];
			} else if (wc>=0x02c0 && wc<0x02e0) {
				Out[0]=cp1255_page02[wc-0x02c0];
			} else if (wc>=0x05b0 && wc<0x05f8) {
				Out[0]=cp1255_page05[wc-0x05b0];
			} else if (wc>=0x2008 && wc<0x2040) {
				Out[0]=cp1255_page20[wc-0x2008];
			} else if (wc==0x0192) {
				Out[0]=0x83;
			} else if (wc==0x20aa) {
				Out[0]=0xa4;
