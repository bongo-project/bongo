...
static const
unsigned short ISO8859_5_UTF8[96] = {
  /* 0xa0 */
...
static int
ISO8859_5_Decode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	...
	while (Len<Codec->Len) {
		...
		if (*In<0xa0) {
			*Out=*In;
			...
			UTF8EncodeChar(ISO8859_5_UTF8[*In-0xa0], Out, Out, NextCodec->Len);
			Len++;
			In++;
		}
	}
...
static int 
ISO8859_5_Encode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	unsigned char	*In=Codec->Start;
	unsigned char	*Out=NextCodec->Start+NextCodec->Len;
	unsigned char	*OutEnd=NextCodec->End;
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x0080) {
			...
			if (wc < 0x00a0){
				...
				Out[0] = ISO8859_5_CP00[wc-0x00a0];
			} else if (wc >= 0x0400 && wc < 0x0460) {
				Out[0] = ISO8859_5_CP04[wc-0x0400];
			} else if (wc == 0x2116) {
				Out[0] = (unsigned char)0xf0;
			} else {
				Out[0] = '?';
			}
			Out++;
