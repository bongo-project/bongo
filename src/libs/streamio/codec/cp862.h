...
#if CP862_CODEC_COMPLETE

static int
CP862_Decode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x80) {
			*Out=*In;
			...
			UTF8EncodeChar(cp862_2uni[In[0]-0x80], Out, Out, NextCodec->Len);
			Len++;
			In++;
		}
	}
...
static int
CP862_Encode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	unsigned char	*In=Codec->Start;
	unsigned char	*Out=NextCodec->Start+NextCodec->Len;
	unsigned char	*OutEnd=NextCodec->End;
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x0080) {
			...
			if (wc>=0x00a0 && wc<0x0100) {
				Out[0]=cp862_page00[wc-0x00a0];
			} else if (wc>=0x0390 && wc<0x03c8) {
				Out[0]=cp862_page03[wc-0x0390];
			} else if (wc>=0x2218 && wc<0x2268) {
				Out[0]=cp862_page22[wc-0x2218];
			} else if (wc>=0x2500 && wc<0x25a8) {
				Out[0]=cp862_page25[wc-0x2500];
			} else if (wc>=0x2320 && wc<0x2322) {
				Out[0]=(unsigned char)(wc-0x222c);
			} else if (wc==0x0192) {
				Out[0]=0x9f;
			} else if (wc==0x207f) {
				Out[0]=0xfc;
