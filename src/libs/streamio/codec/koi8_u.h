...
static const
unsigned short KOI8_U_UTF8[128] = {
  /* 0x80 */
...
static int
KOI8_U_Decode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x80) {
			*Out=*In;
			...
			UTF8EncodeChar(KOI8_U_UTF8[*In - 0x80], Out, Out, NextCodec->Len);
			In++;
			Len++;
		}
	}
...
static const
unsigned char KOI8_U_CP23[8] = {
  0x93, 0x9b, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, /* 0x20-0x27 */
};

static const
unsigned char KOI8_U_CP25[168] = {
...
static int
KOI8_U_Encode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	unsigned char	*In=Codec->Start;
	unsigned char	*Out=NextCodec->Start+NextCodec->Len;
	unsigned char	*OutEnd=NextCodec->End;
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x0080) {
			...
			if (wc >= 0x00a0 && wc < 0x00f8) {
				*Out = KOI8_U_CP00[wc - 0x00a0];
			} else if (wc >= 0x0400 && wc < 0x0498) {
				*Out = KOI8_U_CP04[wc - 0x0400];
			} else if (wc >= 0x2218 && wc < 0x2268) {
				*Out = KOI8_U_CP22[wc - 0x2218];
			} else if (wc >= 0x2320 && wc < 0x2328) {
				*Out = KOI8_U_CP23[wc - 0x2320];
			} else if (wc >= 0x2500 && wc < 0x25a8) {
				*Out = KOI8_U_CP25[wc - 0x2500];
			} else {
				*Out='?';
			}
			Out++;
