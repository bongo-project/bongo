...
static int
CP1256_Decode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x80) {
			*Out=*In;
			...
			UTF8EncodeChar(cp1256_2uni[In[0]-0x80], Out, Out, NextCodec->Len);
			Len++;
			In++;
		}
	}
...
static int
CP1256_Encode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	unsigned char	*In=Codec->Start;
	unsigned char	*Out=NextCodec->Start+NextCodec->Len;
	unsigned char	*OutEnd=NextCodec->End;
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x0080) {
			...
			if (wc>=0x00a0 && wc<0x0100) {
				Out[0]=cp1256_page00[wc-0x00a0];
			} else if (wc>=0x0150 && wc<0x0198) {
				Out[0]=cp1256_page01[wc-0x0150];
			} else if (wc>=0x0608 && wc<0x06d8) {
				Out[0]=cp1256_page06[wc-0x0608];
			} else if (wc>=0x2008 && wc<0x2040) {
				Out[0]=cp1256_page20[wc-0x2008];
			} else if (wc==0x02c6) {
				Out[0]=0x88;
			} else if (wc==0x20ac) {
				Out[0]=0x80;
