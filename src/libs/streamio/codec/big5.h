...
static int
BIG5_Decode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	...
	while (Len<Codec->Len) {
		...
		if (In[0] < 0x80) {
			...
			CheckInStream(2);
			if ((In[0] >= 0xa1 && In[0] <=0xc7) || (In[0]>=0xc9 && In[0]<=0xf9)) {
				if ((In[1]>=0x40 && In[1]<0x7f) || (In[1]>=0xa1 && In[1]<0xff)) {
					unsigned int	Index = 157 * (In[0]-0xa1) + (In[1]-(In[1]>=0xa1 ? 0x62 : 0x40));
					unsigned short	wc		= 0xfffd;

					if (Index<6280) {
						if (Index<6121) {
							wc=big5_2uni_pagea1[Index];
						}
					} else {
						if (Index<13932) {
							wc=big5_2uni_pagec9[Index-6280];
						}
					}
					if (wc != 0xfffd) {
...
static int
BIG5_Encode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	unsigned char	*In=Codec->Start;
	unsigned char	*Out=NextCodec->Start+NextCodec->Len;
	unsigned char	*OutEnd=NextCodec->End;
