...
static const
unsigned short ISO8859_2_UTF8[96] = {
  /* 0xa0 */
...
static int
ISO8859_2_Decode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	...
	while (Len<Codec->Len) {
		...
		if (*In<0xa0) {
			*Out=*In;
			...
			UTF8EncodeChar(ISO8859_2_UTF8[*In - 0xa0], Out, Out, NextCodec->Len);
			Len++;
			In++;
		}
	}
...
static const
unsigned char ISO8859_2_CP00[224] = {
...
static const
unsigned char ISO8859_2_CP02[32] = {
...
static int
ISO8859_2_Encode(StreamStruct *Codec, StreamStruct *NextCodec)
{
	unsigned char	*In=Codec->Start;
	unsigned char	*Out=NextCodec->Start+NextCodec->Len;
	unsigned char	*OutEnd=NextCodec->End;
	...
	while (Len<Codec->Len) {
		...
		if (*In<0x0080) {
			...
			if (wc < 0x00a0){
				...
				Out[0] = (unsigned char)ISO8859_2_CP00[wc-0x00a0];
			} else if (wc >=0x02c0 && wc < 0x02e0) {
				Out[0] = (unsigned char)ISO8859_2_CP02[wc-0x02c0];
			} else {
				Out[0] = '?';
			}
			Out++;
