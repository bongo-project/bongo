...
class Message:
    ...
    def _read_charset(self) :
        charset = self.get_param('charset') or 'us-ascii'
        encoding = self['Content-Transfer-Encoding']
        return bongo.external.email.charset.Charset(charset, encoding)
    ...
    def get_filename(self, failobj=None):
        ...
        if filename is missing:
            return failobj
        ret = utils.collapse_rfc2231_value(filename).strip()
        return ret
    ...
    # I.e. def walk(self): ...
    from bongo.external.email.iterators import walk
